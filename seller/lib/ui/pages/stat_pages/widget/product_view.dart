...
class ProductsIncome extends StatefulWidget {
  ...
  @override
  State<ProductsIncome> createState() => _ProductsIncomeState();
}

class _ProductsIncomeState extends State<ProductsIncome> {
  @override
              ...
              child: Padding(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        statiscService.productViews!.isEmpty
                            ? Padding(
                                padding: const EdgeInsets.all(normal_100),
                                child: Row(
                                  children: [
                            ...
                            : builProductRevenuesBarChart(
                                statiscService.productViews!),
                      ]))));
    });
  }

  Widget builProductRevenuesBarChart(
    List<ProductView> productsIncome,
  ) {
    return Column(
      children: [
...
/*
  ...
  Widget buildBarChart(List<ProductView> productsIncome) {
    return Padding(
      padding: const EdgeInsets.all(normal_100),
      child: Container(
        height: 200,
...
SideTitles _bottomTitles(List<StoreRevenue> storesRevenues) {
  ...
  List<BarChartGroupData> _chartGroups(List<ProductView>? productsViews) {
    return productsViews?.asMap().entries.map((entry) {
          final int index = entry.key;
          final ProductView productViews = entry.value;
          final double numberOfSales = productViews.views.toDouble();
