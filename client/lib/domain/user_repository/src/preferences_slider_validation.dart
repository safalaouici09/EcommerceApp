...
class PreferencesSliderValidation with ChangeNotifier {
  String? _id;
  ...
  PreferencesSliderValidation.setStore(Store store) {
    _id = store.id;
  ...
  Map<String, dynamic> toFormData() {
    ...
    try {
      ...
      Map<String, dynamic> _formData = {
        ...
        "tags": json.encode(fdStoreRayons),
      };
  ...
  Future getInitStoreCategories() async {
    ...
    var credentialsBox = Boxes.getCredentials();
    String _token = credentialsBox.get('token');
    String? user_id = credentialsBox.get('id');
    ...
    try {
      if (user_id == null) {
        ...
        dio.options.headers["token"] = "Bearer $_token";
        var res =
            await dio.get(BASE_API_URL + '/storeCategory/client/' + user_id);
        if (res.statusCode == 200) {
          print(res.data);
          _storeCategories = [];
          _storeCategories!
