...
class RegionChart extends StatelessWidget {
  final List<RegionView> regionViews;
  final int totalViews;

  RegionChart({required this.regionViews, required this.totalViews});
  ...
  List<PieChartSectionData> getChartSections() {
    if (regionViews.isEmpty || totalViews == 0) {
      // Return an empty list if the data is not available or invalid
      return [];
    }

    return List.generate(regionViews.length, (index) {
      final isTouched = index == touchedIndex;
      final double fontSize = isTouched ? 18 : 12;
      final double radius = isTouched ? 60 : 70;
      final double percentage = (regionViews[index].views / totalViews) * 100;
      ...
      final String regionName = regionViews[index].regionName;
